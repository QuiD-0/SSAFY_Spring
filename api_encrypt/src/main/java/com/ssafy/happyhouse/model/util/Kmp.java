...
public class Kmp {
    ...
    public static List<Integer> multiKmp(List<NoticeDto> parents, String[] keywords){
        ...
        for (int i = 0; i < parents.size(); i++) {
            ...
            for (int j = 0; j < keywords.length; j++) {
                if(!kmp(parents.get(i).getTitle(), keywords[j], tableList.get(j))) {
                    isCorrect = false;
                    break;
                }
            }
            if (isCorrect) {
                result.add(parents.get(i).getNoticeId());
            }
        }
        return result;
    }
